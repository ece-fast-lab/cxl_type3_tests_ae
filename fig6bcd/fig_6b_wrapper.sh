...
test_compose_post() {
    ./wrk -D exp -t $THREADS -c $CONNECTONS -d $DURATION -L -s \
        ./scripts/social-network/compose-post.lua http://localhost:8080/wrk2-api/post/compose -R $1 \
        > ./fig_6b/composePost_${NODE}_qps${QPS}_t${THREADS}_c${CONNECTONS}_d${DURATION}sec.txt
}
